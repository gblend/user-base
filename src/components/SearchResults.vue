<template>
  <div
    ...
    v-for="(filteredUser, index) in filteredUsers"
    :key="index"
  >
    <div
      class="w-full mx-auto rounded-xl shadow-xl sm:py-4 flex sm:space-y-0 sm:space-x-6 bg-cst_white-300"
    >
      <div class="lg:w-11/12 mx-auto place-self-center flex">
        <div class="flex w-full py-2 lg:py-0">
          <div class="pr-0.5 lg:pr-4 ml-1 lg:ml-2">
            <div
              class="border-4 lg:border-5 borde-cst_teal-200 rounded-full max-h-16 w-16 lg:max-h-20 lg:w-20"
            >
              <img
                class="block mx-auto sm:mx-0 sm:flex-shrink-0 w-full h-full rounded-full"
                :src="`${filteredUser.picture.thumbnail}`"
                alt="User Avatar"
              />
            </div>
          </div>
          <div class="w-full flex flex-col sm:text-left">
            <div class="lg:space-y-1">
              ...
              >
                ...
                {{ filteredUser.location?.city }},
                {{ filteredUser.location?.state }}
              </p>
            </div>
            <div
            ...
            >
              ...
              >
                <span
                  class="space-x-0.5 lg:space-x-1 flex justify-between items-center text-xs lg:text-base"
                >
                  <i
                ...
                <span
                  class="flex items-center justify-between space-x-0.5 lg:space-x-1 mt-1"
                >
                  <span class="text-cst_gray-200">
                    <svg
...
export default {
  name: "SearchResults",
  computed: {
    ...mapGetters(["categoryName", "filteredUsers", "searchTerm"]),
  },

  methods: {
    ...mapActions(["SET_CATEGORY_NAME", "filterUsers"]),
  },

  watch: {
    searchTerm() {
      if (this.searchTerm.length <= 1 || !this.searchTerm.length) {
        this.$router.replace({ name: "AllUsers" });
      } else {
        this.filterUsers(this.searchTerm).then(() => {});
      }
    },
  },

  mounted() {
    store.commit("SET_CATEGORY_NAME", "Filtered Results");
  },
};
</script>
